...
lock-analysis skill's job. If it is not available, say so and record section approvals in the pull request
instead — but do not do its arithmetic in your head and call the result a lock.
...
2. [Analysing A Use Case](analysing-a-use-case.md), in full.
3. The template, in full.
4. The parent use case's `ANALYSIS.md`, if this use case expands one of its operations. Its types, and the
   operation's start and end space, are your inheritance.
5. Any other `ANALYSIS.md` sharing a type with this one, for what it already declares.
