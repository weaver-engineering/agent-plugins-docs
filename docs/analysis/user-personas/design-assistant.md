...
human-judgement points (Per-Gap Ideation's "how might we," the quick sanity check, an unexpected side effect's
resolution, the final Human Review), where the Architect remains primary and the Assistant defers rather than
guesses.
...
1. Skills the process refers to (`docs/claude-skills/`) not yet existing to call — it has to do that step's work
   directly and say plainly what's missing, rather than being able to rely on them.
2. Having no persistent memory between sessions or invocations — the same gap The Architect's Assistant has,
   made more costly here because Design is a long, genuinely iterative, multi-session process.
3. Entry-state ambiguity it can't resolve without the Architect actively present, the same way any elicitation
   dialogue requires a human in the loop.
4. A reconciliation finding whose resolution isn't its call to make — an unexpected side effect might mean the
