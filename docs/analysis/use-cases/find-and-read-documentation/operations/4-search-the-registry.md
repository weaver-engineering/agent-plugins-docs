...
| Ordinal | Value | Means |
|---|---|---|
| 1 | `does-not-match` | nothing registered in the scope answers the term |
| 2 | `matches` | something does |
...
| Ordinal | Value | Means |
|---|---|---|
| 1 | `none-named` | the request names no scope, and the search runs against the registry the Agent is standing in, which the answer names by its location because the Agent gave it no name |
| 2 | `some-named` | the request names one or more registries, and the answer names them back by the names it was given |
| 3 | `all-named` | the request asks for every registry the Agent can reach, and the answer names each of them back |
...
| Value | Means |
|---|---|
| `references` | each result is an addressable reference, its relevance and the word count that relevance was measured against |
| `preview` | each result additionally carries the opening lines of its own prose |
...
| Value | Means |
|---|---|
| `absent` | the caller names no cap, and the answer is bounded by the default |
| `<1` | a cap below one — a request for no results, which is not a search |
| `given` | a cap within range, and the answer is bounded by that |
| `>100` | a cap above the maximum the operation will honour |

### 2.3 `preview-length` Values

| Value | Means |
|---|---|
| `absent` | the caller names no length, and each preview is bounded by the default |
| `<1` | a preview of no lines, which is a preview the caller did not ask for |
| `given` | a length within range, and each preview is bounded by that |
| `>10` | a length above the maximum the operation will honour |

### 2.4 `rendering` Values

| Value | Means |
|---|---|
| `human` | the answer is written to be read |
| `machine` | the caller declares it will parse the answer |

### 2.5 Pruning Rules

**A number outside its range is refused, not clamped.** `max-results` • `<1` or • `>100`, and
`preview-length` • `<1` or • `>10`, each end the request before anything is searched. Clamping would hand the
caller an answer bounded by a number they did not choose and could not see, which is the one thing a bounded
answer must never do.

**A length named without previews is not invalid.** `preview-length` and `mode` are independent knobs, so
naming a length while asking for references is an ordinary thing for a caller to do. The results come back in
full, with a warning that the length was not used — failing would withhold a deliverable answer over a
parameter that changed nothing about it, and silence would leave the caller believing it applied.
...
| State | Means |
|---|---|
| `available` | the scope reaches one or more registries that can be searched |
| `unavailable` | it does not — a named scope no registry answers to, or no searchable registry where the Agent is standing |
...
| State | Means |
|---|---|
| `document` | a whole document answers, carrying a relevance, and none of its sections answers separately |
| `section` | a section answers, and carries the document and every section above it |
...
| State | Means |
|---|---|
| `within` | everything that answered fits the cap in force |
| `beyond` | more answered than the cap admits, so the lowest-scoring are dropped |

### 3.5 `extent` States

| State | Means |
|---|---|
| `within` | an answering node's prose ends before the preview length in force |
| `beyond` | it runs past that length |
...
**Nothing is searched where the target is unavailable.** With `target` • `unavailable` the other three
dependencies have no state at all, and neither does `term` — there is nothing to match against.

**Nothing is shaped, counted or measured where nothing answered.** With `term` • `does-not-match`, `shape`,
`volume` and `extent` likewise have no state.

**Nothing is measured where nothing is previewed.** `extent` is a question about previewed prose, so it has no
state where `mode` is `references`.

## 4 The Condition Space

The product of §1, §2 and §3 once §1.3, §2.5 and §3.6 have pruned it. A node carrying `&` has the dimensions
beneath it still to settle; a node carrying `-->` is a leaf, and names the result §5 requires of it.

**The request is checked before anything is reached.**

* **max-results**: *<1* · *>100* --> `refused`
* **preview-length**: *<1* · *>10* --> `refused`

**Then the scope, and what it reaches.**

* **scope**: *none-named* &
  * **target**: *unavailable* --> `no local registry`
  * **target**: *available* &
    * *as below, and the answer names the registry by its location*
...
`rendering` is held at every leaf and drawn at none: both values carry the same answer in the same order, so it
multiplies the fixtures and not the cells. `max-results` • *absent* against • *given*, and `preview-length`
• *absent* against • *given*, are likewise held rather than drawn wherever nothing was cut against them — a
number nothing reached is a number that changed nothing.

**A length named without previews is drawn nowhere and warned about everywhere.** `preview-length` • *given*
with `mode` • *references* adds a warning to whatever result the leaf already names, rather than a result of
its own (§2.5).

**What this tree still owes.** `volume` and `extent` are relations between what the corpus holds and what the
request asked for, not states the corpus has on its own, so they are drawn under the parameters they answer to
rather than crossed with them. Whether `shape` needs a cell at all, or only a fixture, is the same question:
nothing below it varies by which kind of node answered. Settling both is the next unit of work.
