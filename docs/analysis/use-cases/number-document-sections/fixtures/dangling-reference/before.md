...
See §2 for the interface. The retired behaviour was described in §9. The [model's own §2.3](widget-model.md) belongs to another document;
[retry §2.1](widget-store-guide.md) and [retry again §2.1](./widget-store-guide.md) belong to this one.
...
What the store does when it will not accept a widget. The worked example in §Appendix.5 shows one.

### 2.1 Retry

Retrying is the caller's responsibility.

``` 2.1.a Retry Invocation
