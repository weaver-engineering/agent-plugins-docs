...
| [Design Directory](design/design-assistant/serialization/SERIALIZATION.md#3-scope) | A directory containing a `DESIGN.yaml`, plus its subdirectories, excluding any that is itself one. The marker declares the namespace, so scope is decidable before parsing. |
